...
My reply came in two messages: "Haha I merely said that there's real innovation in harnesses that shouldn't be ignored!" and then "Love pi though! :homer-disappear:"

Zack: "WHAT DO YOU MEAN MAN"
...
My city just didn't move with me. Fleet only knew how to read Claude Code's hooks, case only dispatched Claude, and my tmux notifications never lit up for a pi pane. Every hour in pi was an hour outside the tooling I'd spent months sharpening.

The most telling commit in my dotfiles is from May 28th: `feat: cut tmux + Claude Code over to fleet`. Same day I published the Fleet post. That was the peak of the city, and the moment pi became structurally impossible for me.

So when Zack asked, I told him it wasn't worth it. I wasn't lying, the cost was real. I just had the cause wrong.
...
To know whether a Claude Code agent is working, waiting, or done, Fleet fuses three signals, none of which it can trust alone. Bash hooks writing status files (fast, structured, arrive out of order). A per-pane JSONL event log (knows intent, lags). And `tmux capture-pane` (ground truth, costs ~50ms, and Claude's spinner _animates_, so a fixed glyph match misses half the frames). The whole lesson of that project was that each layer is authoritative only for what it can be trusted on, because nothing in the stack could just ask the agent what it was doing.
...
I also asked pi, seriously, whether I should package all of it as a distro for other people. It spun up an 18-agent research workflow, came back, and told me no. The distro slot in that ecosystem is already crowded with no clear winner, and capability packages beat curated configs by roughly 400×. Then it kept going and told me two things I hadn't asked about: I was loading ~2,600 lines of broken code into every single session, and my `trust.json` was committed to a public repo with my absolute paths in it.
...
And there's `bosun`: a pi session that turns into a liaison over a fleet of headless pi crewmates. It spawns tasks, relays progress, approves delivery. One night I pointed it at seven agents at once and went to bed.
