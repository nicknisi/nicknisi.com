...
Ideation is a Claude Code plugin that turns messy brain dumps into structured, executable implementation specs. <a href="#try-it" class="underline font-medium text-vivid-light-blue-100 hover:text-white">Skip to install instructions &rarr;</a>
</Callout>
...
<Callout title="Why not npx skills?" variant="info">
`npx skills add nicknisi/claude-plugins` only installs the `/ideation` skill. Installing the full plugin includes the companion `/execute-spec` skill, which handles handing off your spec to a fresh session for implementation.
</Callout>
