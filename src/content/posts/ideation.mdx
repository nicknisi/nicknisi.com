...
<Callout title="TL;DR" variant="info">
	...
	<a href="#try-it" class="text-vivid-light-blue-100 font-medium underline hover:text-white">
		Skip to install instructions &rarr;
	</a>
</Callout>
...
<Callout title="Why not npx skills?" variant="info">
	`npx skills add nicknisi/claude-plugins` only installs the `/ideation` skill. Installing the full plugin includes the
	companion `/execute-spec` skill, which handles handing off your spec to a fresh session for implementation.
</Callout>
