...
<Callout title="Check out my recent abstracts" variant="info">
I created small _marketing pages_ for the talks I've given in 2024. These include all resources and the abstracts I submitted to the conference organizers. You can find them at
  [nicknisi.com/compiler-talk](https://nicknisi.com/compiler-talk) and [nicknisi.com/state-talk](https://nicknisi.com/state-talk).
</Callout>
...
Conference organizers aren't kidding when they say they have to make hard choices. At [NEJS Conf](https://nejsconf.com), we would regularly get more than 300 submissions and would have to whittle that list down to nine each year.
...
If you're a fan of [Community](https://en.wikipedia.org/wiki/Community_(TV_series)) or [Rick and Morty](https://en.wikipedia.org/wiki/Rick_and_Morty), you might have heard of [Dan Harmon](https://en.wikipedia.org/wiki/Dan_Harmon). He famously developed a framework for consistently telling great stories. It's called the [Story Circle](https://en.wikipedia.org/wiki/Dan_Harmon#%22Story_circle%22_technique) and it's an eight step process adapted from [Joseph Campbell's](https://en.wikipedia.org/wiki/Joseph_Campbell) [monomyth](https://en.wikipedia.org/wiki/Hero%27s_journey). Here are the steps:
...
7. **Apply Knowledge (Return)** - Describe the results and how the solution impacted the problem or was integrated into your project, specifically.
8. **Results & Insights (Change)** - Conclude the journey with your perspective on the lessons learned and how you or your workflow may have changed.
...
In my talk, [Unleashing the TypeScript Compiler](/compiler-talk), I used the Story Circle to structure my talk. I start with the status quo (1) of a project I worked on, developing a TypeScript/React application using [Material-UI](https://mui.com) and the challenges we faced with that project (2). I describe the solutions we tried as a
way to work around the issues (3) before disrupting things by exploring the TypeScript compiler and the world of codemods (4).
