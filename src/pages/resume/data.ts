import Resume from './interfaces.js';

const data: Resume = {
	basics: {
		name: 'Nick Nisi',
		label: 'Staff Software Engineer',
		image: 'resume/nicknisi.jpg',
		email: 'nick@nisi.org',
		phone: '+14022161612',
		url: 'https://nicknisi.com',
		// summary: "With more than 15 years in the industry as a dedicated JavaScript and TypeScript specialist, I actively advocate for community through teaching and enhancing developer tooling. I'm a panelist on the JS Party podcast and I proudly organized Nebraska's sole JavaScript conference.",
		summary:
			'JavaScript/TypeScript expert with 15+ years of experience leading developer initiatives, mentoring teams, writing robust software, and organizing community events. Proven track record in enhancing user experience and driving cross-functional collaborations.',
		location: {
			address: '',
			postalCode: '',
			city: 'Omaha',
			countryCode: 'US',
			region: 'Nebraska',
		},
		profiles: [
			{
				network: 'GitHub',
				username: 'nicknisi',
				url: 'https://github.com/nicknisi',
			},
			{
				network: 'LinkedIn',
				username: 'nicknisi',
				url: 'https://linkedin.com/in/nicknisi',
			},
			{
				network: 'Threads',
				username: 'nicknisi',
				url: 'https://threads.net/nicknisi',
			},
			{
				network: 'Mastodon',
				username: '@nicknisi@nicknisi.com',
				url: 'https://nicknisi.com/@nicknisi',
			},
			{
				network: 'Twitter',
				username: 'nicknisi',
				url: 'https://twitter.com/nicknisi',
			},
			{
				network: 'BlueSky',
				username: '@nicknisi.com',
				url: 'https://bsky.app/profile/nicknisi.com',
				hidden: true,
			},
		],
	},
	work: [
		{
			name: 'Meta',
			image: 'resume/meta.svg',
			remote: true,
			description: 'Global technology company focusing on social networking services and virtual reality.',
			location: 'Menlo Park, CA',
			position: 'Software Engineer',
			url: 'https://meta.com',
			startDate: '2024-02-05',
			highlights: [
				'Drove feature development, including an onboarding wizard, enhancing user experience and contributing to increased revenue',
				'Spearheaded cross-functional initiatives, such as major bug fixes and cross-platform API alignment, fostering collaboration and efficiency',
				'Organized a successful internal UI summit for front-end developers, promoting knowledge sharing and innovation',
				'Leveraged expertise in React, JavaScript/Flow, and Hack/PHP to deliver high-quality, scalable solutions',
				'Strengthened collaboration between Ads Manager Desktop and cross-functional teams through effective communication and mentorship',
			],
		},
		{
			name: 'C2FO',
			image: 'resume/c2fo.svg',
			description: 'A financial technology company that provides working capital solutions to businesses.',
			remote: true,
			location: 'Kansas City, KS',
			position: 'Staff Software Engineer',
			url: 'https://c2fo.com',
			startDate: '2021-12-09',
			endDate: '2024-01-12',
			summary: 'A tinkering archetype focused on the developer experience and front end tooling.',
			highlights: [
				'Part of the Developer Experience team',
				'Maintainer of design system and react component library using MUI and Storybook',
				'Lead front end chapter meetings to discuss new paradigms and best practices',
				'Lead on project building a complex builder application using multiple state machines',
				'oversaw conversion from react-scripts to vite in the frontend monorepo',
			],
		},
		{
			name: 'C2FO',
			image: 'resume/c2fo.svg',
			description: 'A financial technology company that provides working capital solutions to businesses.',
			remote: true,
			location: 'Kansas City, KS',
			position: 'Senior Software Engineer II',
			url: 'https://c2fo.com',
			startDate: '2020-05-26',
			endDate: '2021-12-09',
			summary: 'Focus on the front end of the money-making product.',
			highlights: [
				'Oversaw and implemented the upgrade of React and TypeScript in a monorepo with more than 70 apps without introducing regressions',
				'Implemented tooling that used AST walking to identify areas of application that were missing internationalization',
				'Maintainer of React component library for internal design system',
				'Full-stack development with TypeScript, implementing GraphQL APIs with NestJS, and consuming them in a React front end',
			],
		},
		{
			name: 'Sitepen',
			image: 'resume/sitepen.svg',
			remote: true,
			location: 'Palo Alto, CA',
			position: 'Senior Software Engineer',
			url: 'https://sitepen.com',
			startDate: '2013-07-08',
			endDate: '2020-05-04',
			summary:
				'An open source contributor, instructor, and lead developer working with some of the top clients in the world.',
			highlights: [
				'Open Source Contributor, ensuring success on many company and community open source initiatives including Dojo, Intern, TypeDoc, and CodeSandbox',
				'Instructor, teaching 2-5 day workshops online and in-person on topics including TypeScript, JavaScript fundamentals, and Dojo.',
				'Lead Developer, managing teams of up to three developers on successful customer-facing projects for world-class clients including hotels, government agencies, and banks.',
				'Lead Support Engineer, providing timely and thorough answers to technical questions from developers about code, bugs, testing, and architecture.',
			],
		},
		{
			name: 'Flywheel',
			image: 'resume/flywheel.svg',
			remote: false,
			location: 'Omaha, NE',
			position: 'Senior Software Engineer',
			url: 'https://getflywheel.com/',
			startDate: '2018-01-23',
			endDate: '2018-04-23',
			summary: 'A developer on the Fly-Tools team',
			highlights: [
				'Developer on the Fly-tools team, delivering tools to internal users to ensure success of a thriving startup and web-hosting company',
				'Introduced TypeScript to the company via the development of a small application for linking support tickets',
			],
		},
		{
			name: 'C2FO (Pollenware)',
			image: 'resume/c2fo.svg',
			description: 'A financial technology startup that provides working capital solutions to businesses.',
			remote: true,
			location: 'Kansas City, KS',
			position: 'Senior Software Engineer',
			url: 'https://c2fo.com',
			startDate: '2011-11-11',
			endDate: '2013-07-08',
			summary: '',
			highlights: [
				'Part of core development team delivering results which lead to $9.1 million funding round lead by Union Square Ventures.',
				'Designed and developed frontend architecture on c2fo.com with a focus on the responsive design and using Backbone, RequireJS, Handlebars, and Less to create a friendly single page application.',
			],
		},
		{
			name: 'Freelancing',
			remote: false,
			hidden: true,
			position: 'Freelance Developer',
			url: 'https://nicknisi.com',
			startDate: '2012-04-01',
			endDate: '2012-06-30',
			summary: '',
			highlights: [
				'Architected and implemented a proof of concept application for a client utilizing advanced HTML5 and canvas technologies.',
				'Demonstrated the feasibility of using these technologies to make a custom, drag and drop interface that is functional on desktop and iPad/tablet devices.',
				'Utilized Backbone Models and Views to manage rendering and state of shapes and images drawn to a canvas.',
				'Connected to simple REST interface written in Ruby on Rails to save and load state.',
				'Delivered on time and successfully.',
			],
		},
		{
			name: 'Lockheed Martin',
			image: 'resume/lockheed-martin.svg',
			remote: false,
			location: 'Omaha, NE',
			position: 'Software Engineer / Englineering Leadership Development Program',
			url: 'https://lockheedmartin.com',
			startDate: '2009-01-23',
			endDate: '2011-11-09',
			hidden: true,
			summary: '',
			highlights: [
				'Accepted into Engineering Leadership Development Program (ELDP) based on job performance, panel interviews, and essays.',
				'Development lead for maintenance drop focusing on performance enhancement and usability, where duties include leading a team of three engineers, supporting builds and turnovers, and cross functional coordination to facilitate the engineering life cycle.',
				'Lead Developer of a system demo application using Flash and driven by XML files which included audio narration, videos, and interactive elements for the customer and met regularly with a customer working group to ensure the demo met their needs.',
				'Chief Architect and lead developer of a metrics reporting web application written in Groovy/Grails that reads data from another application database and generates reports using JasperReports.',
			],
		},
	],
	education: [
		{
			institution: 'University of Nebraska at Omaha',
			area: 'Computer Science',
			studyType: 'Bachelor of Science',
			startDate: '2005-05-23',
			endDate: '2008-12-17',
		},
	],
	publications: [
		{
			name: 'Unleashing the TypeScript Compiler',
			releaseDate: '2024-03-19',
			publisher: 'The Monthly Dev',
			url: 'https://www.youtube.com/SELZVza_DoM?t=2719',
			summary: 'A brief introduction to querying and modifying TypeScript source using the compiler API.',
			hidden: true,
		},
		{
			name: 'Componentizing Application State',
			publisher: 'THAT Conference Texas',
			releaseDate: '2024-01-30',
			url: 'https://www.youtube.com/watch?v=4-msZqqrN1A',
			summary: 'A talk about treating your application state as a component using React and XState.',
		},
		{
			name: 'TalkScript with the TypeScript Team',
			publisher: 'TypeScript Conf',
			releaseDate: '2021-09-10',
			url: 'https://www.youtube.com/watch?v=gfn-aKykyAM',
			summary: 'A live show with the TypeScript team at Microsoft discussing the future of TypeScript.',
		},
		{
			name: 'Rules to Git By',
			publisher: 'NebraskaJS',
			releaseDate: '2021-10-21',
			url: 'https://www.youtube.com/watch?v=aHKn59spc6M',
			summary: 'An advanced dive into Git and how to use it effectively.',
			hidden: true,
		},
		{
			name: 'No Time for Types',
			publisher: 'JSConf US',
			releaseDate: '2018-08-21',
			url: 'https://www.youtube.com/watch?v=qGK541P2xII',
			summary: 'A talk about incrementally adding TypeScript to your JavaScript project.',
		},
	],
	skills: [
		{ name: 'TypeScript', level: 'expert' },
		{ name: 'JavaScript', level: 'expert' },
		{ name: 'React', level: 'expert' },
		{ name: 'Next.js', level: 'expert' },
		{ name: 'Node.js', level: 'expert' },
		{ name: 'GraphQL', level: 'expert' },
		{ name: 'Leadership', level: 'expert' },
	],
	languages: [
		{
			language: 'English',
			fluency: 'Native speaker',
		},
	],
	projects: [
		{
			name: 'JS Party',
			image: 'resume/jsparty.png',
			startDate: '2018-04-06',
			description: 'A weekly celebration of JavaScript and the web.',
			highlights: ['Panelist from 2018 through present'],
			keywords: ['JavaScript', 'TypeScript', 'Community', 'Podcast'],
			url: 'https://jsparty.fm',
			roles: ['Panelist'],
			type: 'podcast',
			entity: 'Changelog',
		},
		{
			name: 'NebraskaJS',
			image: 'resume/nebraskajs.svg',
			description: 'A community of JavaScript developers in Nebraska.',
			highlights: ['Organized a conference from 2015 through 2019'],
			keywords: ['JavaScript', 'TypeScript', 'Community', 'Meetup'],
			url: 'https://nebraskajs.com',
			roles: ['Organizer', 'Speaker'],
			type: 'meetup',
			entity: 'NebraskaJS',
		},
		{
			name: 'TalkScript',
			image: 'resume/talkscript.svg',
			startDate: '2015-05-01',
			endDate: '2022-12-14',
			description: 'A podcast about the TypeScript language and community.',
			highlights: [
				'Interviwed guests from the TypeScript community',
				'Interviewed Anders Hejlsberg, the creator of TypeScript',
				'Interviewed Ryan Dahl, the creator of Node.js and Deno',
				'Interviewed the TypeScript team at Microsoft',
				'Performed live shows at each TypeScript Conf from 2018 through 2022',
			],
			keywords: ['JavaScript', 'TypeScript', 'Community', 'Podcast'],
			url: 'https://talkscript.fm',
			roles: ['Panelist'],
			type: 'podcast',
			entity: 'Changelog',
		},
		{
			name: 'Dojo',
			description: 'A modern, reactive framework for building efficient and scalable web applications.',
			highlights: [
				'Contributed to the Dojo 1.x framework and its supporting libraries',
				'Contributed to the Dojo 2+ framework and its supporting libraries',
				'Contributed to the Dojo CLI, including a codemod tool to help with upgrading from Dojo 2 to Dojo 3',
			],
			keywords: ['JavaScript', 'TypeScript', 'Framework', 'Open Source'],
			startDate: '2013-07-08',
			endDate: '2020-05-26',
			url: 'https://dojo.io',
			roles: ['Contributor'],
			type: 'oss',
			entity: 'Open Source',
			hidden: true,
		},
		{
			name: 'TypeDoc',
			description: 'A documentation generator for TypeScript projects.',
			highlights: [],
			keywords: ['JavaScript', 'TypeScript', 'Open Source'],
			url: 'https://typedoc.io',
			roles: ['Contributor'],
			type: 'oss',
			entity: 'oss',
			hidden: true,
		},
		{
			name: 'CodeSandbox',
			description: 'An online editor for rapid prototyping and sharing of web applications.',
			highlights: ['First outside contributor of new framework support (Dojo).'],
			keywords: ['JavaScript', 'TypeScript', 'Open Source'],
			url: 'https://typedoc.io',
			roles: ['Contributor'],
			type: 'oss',
			entity: 'Open Source',
			hidden: true,
		},
		{
			name: 'nicknisi dotfiles',
			description: 'A collection of configuration files for my development environment.',
			highlights: ['Maintainer of a popular dotfiles repository with more than 3,000 stars on GitHub'],
			keywords: ['Vim', 'Neovim', 'tmux', 'Zsh', 'Bash', 'Lua', 'Open Source'],
			url: 'https://typedoc.io',
			roles: ['Contributor'],
			type: 'oss',
			entity: 'Open Source',
			hidden: true,
		},
		{
			name: 'NEJS Conf',
			description: 'A conference for JavaScript developers in Nebraska.',
			highlights: ['Organized a conference from 2015 through 2019', 'Speakers from all over the world'],
			startDate: '2015',
			endDate: '2019',
			keywords: ['JavaScript', 'Community', 'Conference'],
			url: 'https://nejsconf.com',
			roles: ['Organizer', 'Emcee'],
			type: 'conference',
			entity: 'NebraskaJS',
			hidden: true,
		},
		{
			name: 'TypeScript Conf',
			startDate: '2018',
			endDate: '2022',
			description: 'A single day, single track conference highlighting TypeScript and the TypeScript community.',
			highlights: ['Emcee from 2018 through present'],
			keywords: ['JavaScript', 'TypeScript', 'Community', 'Conference'],
			url: 'https://tsconf.io',
			roles: ['Emcee'],
			type: 'conference',
			entity: 'SitePen',
			hidden: true,
		},
	],
} as const;

export default data;
